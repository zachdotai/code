...
- If a recent message_hoglet shows a "task tab is not currently open" audit, the hoglet has NOT seen the message yet — it will only be delivered if the operator attaches the hoglet's task tab. Do not repeat the same probe blindly. Either wait for the operator to attach, wait for the run to complete (where the hoglet's findings will surface as a hoglet_summary), or — if the question is genuinely time-sensitive — call write_audit_entry to surface the question to the operator in nest chat instead of re-probing.
