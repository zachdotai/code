...
vi.mock("@features/auth/hooks/authMutations", () => ({
  ...
  useSwitchOrgMutation: () => ({ mutate: switchOrgMutate }),
}));
...
describe("ProjectSwitcher org switcher", () => {
  ...
  it("fires switchOrg.mutate when picking a different org and skips when picking the current one", async () => {
    mockAuthState = {
      cloudRegion: "us",
      currentOrgId: "org-1",
      orgProjectsMap: {
    ...
    expect(switchOrgMutate).toHaveBeenCalledExactlyOnceWith("org-2");
  });
