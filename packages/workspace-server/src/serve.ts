...
// rollup graph, and rollup may import THIS entry chunk from a sibling chunk to
// reach modules they share — executing this file inside the Electron main
// process (or the node-host utility). Only boot the server when this file is
// the spawned child's own entry script; a bare import must stay side-effect
// free, while a genuine spawn with broken env still fails fast below.
const entryScript = process.argv[1];
if (
  entryScript !== undefined &&
  path.basename(entryScript) === "workspace-server.js"
) {
  runWorkspaceServer();
}
...
function runWorkspaceServer(): void {
  ...
  const router = createAppRouter({
    ...
    connectivityService:
      container.get<ConnectivityService>(CONNECTIVITY_SERVICE),
