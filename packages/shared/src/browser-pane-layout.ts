 ...
 * Snapshot-level pane transforms (which also move panes and focus) live in
...
/**
 * Insert an arbitrary subtree next to `targetPaneId` on the `direction` side —
 * `targetPaneId: null` targets the layout root (content-area edge drops).
 * If the insertion axis matches the surrounding split, the node is spliced in
 * as a sibling (n-ary); otherwise the target is wrapped in a new two-child
 * split. The inserted subtree takes an equal share of the affected split;
 * existing siblings scale down proportionally. The result is normalized, so a
 * same-direction split node flattens into its new parent (its children share
 * the inserted slot proportionally). Unknown target → tree returned unchanged.
 */
export function insertNodeInLayout(
  root: PaneLayoutNode,
  ...
  const wrap = (target: PaneLayoutNode): PaneLayoutNode => ({
    type: "split",
    direction: axis,
  ...
  const inserted = (() => {
    if (targetPaneId === null) {
      // Root drop: splice into a same-axis root split, else wrap the tree.
      if (root.type === "split" && root.direction === axis) {
        return spliceSibling(root, before ? 0 : root.children.length, node);
      }
      return wrap(root);
    }

    const insert = (current: PaneLayoutNode): PaneLayoutNode | null => {
      if (current.type === "leaf") {
        return current.paneId === targetPaneId ? wrap(current) : null;
      }
      for (let i = 0; i < current.children.length; i++) {
        ...
        if (
          ...
          return spliceSibling(current, before ? i : i + 1, node);
        }
        const replaced = insert(child);
        if (replaced) {
          const children = current.children.map((c, j) =>
            j === i ? replaced : c,
          );
  ...
  if (inserted == null) return root;
  // The inserted node may itself be a split on the insertion axis (merging a
  // multi-pane tab) — normalize flattens that same-direction nesting.
  return normalizeLayout(inserted) ?? root;
}

/**
 * Insert a new pane next to `targetPaneId` on the `direction` side. The
 * single-leaf specialisation of {@link insertNodeInLayout}.
 */
export function insertPaneInLayout(
  ...
  return insertNodeInLayout(root, targetPaneId, direction, {
    type: "leaf",
    paneId: newPaneId,
  });
}
