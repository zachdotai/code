...
import { genderForName } from "@posthog/host-router/rts-schemas";
...
import { GitBranchDialog } from "@posthog/ui/features/git-interaction/components/GitInteractionDialogs";
import { gitWriteClient } from "@posthog/ui/features/git-interaction/gitInteractionAdapter";
...
} from "@posthog/ui/features/git-interaction/utils/branchCreation";
import {
...
import { CloudGithubMissingNotice } from "@posthog/ui/features/task-detail/components/CloudGithubMissingNotice";
import {
  type WorkspaceMode,
  WorkspaceModeSelect,
...
import { useTaskCreation } from "@posthog/ui/features/task-detail/hooks/useTaskCreation";
...
export function SpawnHogletPanel({ onClose }: SpawnHogletPanelProps) {
  ...
  const handleTaskCreated = useCallback(
    async (task: Task) => {
      try {
        const hoglet = await hostClient.rts.hoglets.recordAdhoc.mutate({
          taskId: task.id,
        });
  ...
  useEffect(() => {
    let cancelled = false;
    hostClient.skills.list.query().then((skills) => {
      if (cancelled) return;
