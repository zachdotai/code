...
export function PlaceNestDialog({
  ...
  const requestDraft = async (
    ...
    try {
      const response = await hostClient.rts.goalDraft.respond.mutate({
        transcript: sendTranscript,
        currentDraft,
  ...
  const handleImportSpec = async () => {
    ...
    try {
      const result = await hostClient.rts.goalDraft.importSpecFile.mutate();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const created = await hostClient.rts.nests.create.mutate({
        name: effectiveName,
