...
/** Thin edge strips over the WHOLE content area for splitting the window
 * root. High collision priority so they beat the outermost panes' 20% edge
 * zones where the two overlap. */
const EDGE = "18px";
...
/**
 * Content-area edge drops that split the window ROOT (the whole UI splits;
 * nav and title bar stay put). Mounted only while a pill drag is live, and
 * suppressed when the drag couldn't produce a new layout (single pane whose
 * only tab is the dragged one — a root split is then equivalent to a pane
 * split of itself).
 */
