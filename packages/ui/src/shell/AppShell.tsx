...
// so the title-bar controls never render under the native controls.
const WINDOWS_TITLEBAR_INSET = 140;

/**
 * The window chrome, rendered OUTSIDE the route tree: title bar (logo,
 * back/forward, PostHog Web), connectivity banner, channels sidebar, global
 * modals/menus, deep-link handlers, and every window-level side effect that
 * must run exactly once. `children` is the routed content (the pane area —
 * a RouterProvider per pane), mounted in the inset content card.
 *
 * The whole shell sits inside a `RouterContextProvider` bound to the FOCUSED
 * pane's router, so every existing router hook in the sidebar and title bar
 * (useAppView, useRouterState, useCanGoBack, Link) reads the focused pane and
 * retargets automatically when focus moves — chrome gets focused-pane
 * semantics without rewriting its hooks.
 */
...
function AppShellChrome({ children }: { children: ReactNode }) {
  ...
  // fit its own controls so the content area flushes left.
  ...
  return (
    // DnD scope for the tab strips' drag-to-reorder (and, with split panes,
    // cross-pane drops) — the provider must sit above every pane.
    <BrowserTabsDndProvider>
      <Flex direction="column" height="100%" className="bg-chrome">
        {/* Full-width title bar: a window-drag region carrying the PostHog
            mark. The left section matches the sidebar width so the content
            area starts flush with the sidebar edge; its padding clears the
            macOS stoplights. */}
        <Flex
          align="center"
        ...
        <Flex flexGrow="1" overflow="hidden">
          ...
          {/* Content sits in a bordered, rounded card inset from the window
              edges — the framed pane from the design. The routed pane area
              (each pane's tab strip + outlet) renders inside it. */}
