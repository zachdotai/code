 ...
 * the persisted href over the pane's active-tab-derived one.
 *
 ...
 * active tab) is the source of truth, exactly like the old hash history
...
function write(map: Record<string, string>): void {
  try {
    ...
    // Quota/unavailable — location restore degrades to the active-tab href.
  }
}
