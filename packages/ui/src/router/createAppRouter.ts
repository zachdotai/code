...
export function createAppRouter(opts: { paneId: string; initialHref: string }) {
  ...
  // Forward-availability tracker. It must live WITH the router (not in
  // component state): the title bar swaps routers when pane focus moves and
  // would otherwise lose the counter. Installed before the persistence
  // subscriber so listeners always observe an up-to-date max index.
