...
//
// Every pane hosts its own router instance (an active tab's panes each render
// their own route). "The" router for imperative navigation is the FOCUSED
// pane's — the active tab's focused pane — resolved here from the tabs mirror.
...
/** The focused pane id: the active tab's `focusedPaneId`, from the mirror. */
export function focusedPaneId(): string | null {
  const snapshot = browserTabsStore.getState().snapshot;
  const win = primaryWindow(snapshot);
  if (!win?.activeTabId) return null;
  const tab = snapshot.tabs.find((t) => t.id === win.activeTabId);
  return tab?.focusedPaneId ?? null;
}

/**
 * The focused pane's router: the active tab's focused pane when a router is
 * registered under that id, else — while the app runs a single router not yet
 * keyed by a real pane id (boot, unit tests) — the sole registered instance.
 * Null before any router exists; callers treat null as "no router, nothing to
 * navigate".
 */
