...
// reaching the router through `paneRouterRegistry` (a leaf module) rather than
// importing `./router` directly, keeps itself out of the route-tree import
// cycle:
...
// route chunks (TDZ on `rootRouteImport`). See paneRouterRegistry.ts.
//
// With split panes every pane hosts its own router; imperative navigation
// targets the FOCUSED pane's router (the registry resolves it from the tabs
// mirror), which is the right semantic for every caller here — sidebar items,
// deep links, notifications, and shortcuts act on the pane being worked in.
//
// Every call degrades to a no-op / empty read when no router is mounted yet
// (early boot, unit tests). These are renderer conveniences — they must never
// throw just because no router has been created.

export function navigateToCode(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/code" });
}

export function navigateToTaskDetail(taskId: string): void {
  void getFocusedRouterOrNull()?.navigate({
...
export function navigateToActivity(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/website/activity" });
}
...
export function navigateToHome(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/code/home" });
}

export function navigateToInbox(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/code/inbox" });
}
...
export function navigateToAgents(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/code/agents" });
}
...
export function navigateToArchived(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/code/archived" });
}

export function navigateToCommandCenter(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/command-center" });
...
export function navigateToSkills(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/skills" });
}

export function navigateToMcpServers(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/mcp-servers" });
}
...
export function navigateToWebsiteNew(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/website/new" });
}

export function navigateToWebsiteHome(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/website/home" });
}

// The Canvas workspace landing (the channels index, where canvases live).
export function navigateToCanvas(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/website" });
}

export function navigateToWebsiteSkills(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/website/skills" });
}

export function navigateToWebsiteMcpServers(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/website/mcp-servers" });
}

export function navigateToWebsiteCommandCenter(): void {
  void getFocusedRouterOrNull()?.navigate({ to: "/website/command-center" });
...
export function isOnSettingsRoute(): boolean {
  return (
    getFocusedRouterOrNull()?.state.matches.some((m) =>
...
export function goBackInHistory(): void {
  getFocusedRouterOrNull()?.history.back();
}
...
export function canGoBackInHistory(): boolean {
  return getFocusedRouterOrNull()?.history.canGoBack() ?? false;
}

export function goForwardInHistory(): void {
  getFocusedRouterOrNull()?.history.forward();
}
...
export function getCurrentMatches() {
  return getFocusedRouterOrNull()?.state.matches ?? [];
}

export function getCurrentLocation() {
  return getFocusedRouterOrNull()?.state.location ?? null;
}
